...
class DataLoader:
    ...
    async def load_data_into_franchise_table(self) -> None:

        current_franchise_dict: dict[int, str] = self._database_client.get_franchise_dict_from_franchise_table()
        ...
        try:
            async with async_playwright() as playwright_obj:
                async with await playwright_obj.chromium.launch(headless=False) as browser:
                    ...
                    for franchise_id, franchise_name in current_franchise_dict.items():
                        try:
                            ...
                            if not stats_list:
                                self._logger.warning(f"No stats found for {franchise_name}, skipping")
                                continue

                            # Insert stats rows with the known player_id
                            await self._database_client.insert_rows_into_franchise_per_season_stats_table(
                                franchise_name_str=franchise_name,
                                stats_list=stats_list,
                            )

                        except Exception as player_error:
                            self._logger.warning(
                                f"Failed to process '{franchise_name}', skipping — reason: {player_error}")
                            await self._web_scraper.navigate_to_base_url(page=page)
                            continue
        finally:
                ...
                async with await playwright_obj.chromium.launch(headless=False) as browser:
                    ...
                    for player_id, player_name in player_dict.items():
                        try:
                            ...
                            self._logger.warning(
                                f"Failed to process '{player_name}', skipping — reason: {player_error}")
                            await self._web_scraper.navigate_to_base_url(page=page)
                            continue
        finally:
            self._logger.info("Closing connection pool")
